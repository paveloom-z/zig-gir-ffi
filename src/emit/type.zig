...
pub fn from(
    ...
    const is_pointer = gir.g_type_info_is_pointer(@"type") != 0;
    const tag = gir.g_type_info_get_tag(@"type");
    const name = switch (tag) {
        ...
        gir.GI_TYPE_TAG_ARRAY => out: {
            const array_type = gir.g_type_info_get_array_type(@"type");
            switch (array_type) {
                gir.GI_ARRAY_TYPE_C => {
                    const param_type_info = gir.g_type_info_get_param_type(@"type", 0);
        ...
        gir.GI_TYPE_TAG_INTERFACE => out: {
            const interface = gir.g_type_info_get_interface(@"type");
